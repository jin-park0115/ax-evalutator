...
| 11 | 최종 제출 | 학생 | 임시저장해 둔 팀 평가·개인 평가 전체를 한 번에 확정한다. 이후 수정 불가 | P-19 |
| 12 | 회차 종료 | 관리자 | 회차 상태를 `finished`로 전환한다 | P-10 |
| 13 | 점수 계산 | 시스템 | 학생 평가 + 튜터 평가를 가중치대로 합산해 팀/개인/최종 점수와 석차를 계산하고, 다음 회차 자동 편성에 쓸 누적 시드도 갱신한다 | P-22 |
